...
type TeamQuery struct {
	...
	withAdmins        *TeamAdminQuery
	withBalanceLogs   *TeamBalanceLogQuery
...
func (_q *TeamQuery) QueryAdmins() *TeamAdminQuery {
...
func (_q *TeamQuery) QueryBalanceLogs() *TeamBalanceLogQuery {
	query := (&TeamBalanceLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TeamQuery) QuerySubscriptions() *UserSubscriptionQuery {
...
func (_q *TeamQuery) WithAdmins(opts ...func(*TeamAdminQuery)) *TeamQuery {
	query := (&TeamAdminClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithBalanceLogs(opts ...func(*TeamBalanceLogQuery)) *TeamQuery {
	query := (&TeamBalanceLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) WithSubscriptions(opts ...func(*UserSubscriptionQuery)) *TeamQuery {
	query := (&UserSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withAdmins; query != nil {
		if err := _q.loadAdmins(ctx, query, nodes,
			func(n *Team) { n.Edges.Admins = []*TeamAdmin{} },
			func(n *Team, e *TeamAdmin) { n.Edges.Admins = append(n.Edges.Admins, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withBalanceLogs; query != nil {
		if err := _q.loadBalanceLogs(ctx, query, nodes,
			func(n *Team) { n.Edges.BalanceLogs = []*TeamBalanceLog{} },
			func(n *Team, e *TeamBalanceLog) { n.Edges.BalanceLogs = append(n.Edges.BalanceLogs, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSubscriptions; query != nil {
		if err := _q.loadSubscriptions(ctx, query, nodes,
			func(n *Team) { n.Edges.Subscriptions = []*UserSubscription{} },
			func(n *Team, e *UserSubscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadAdmins(ctx context.Context, query *TeamAdminQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamAdmin)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teamadmin.FieldTeamID)
	}
	query.Where(predicate.TeamAdmin(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.AdminsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TeamQuery) loadBalanceLogs(ctx context.Context, query *TeamBalanceLogQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamBalanceLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(teambalancelog.FieldTeamID)
	}
	query.Where(predicate.TeamBalanceLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.BalanceLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TeamQuery) loadSubscriptions(ctx context.Context, query *UserSubscriptionQuery, nodes []*Team, init func(*Team), assign func(*Team, *UserSubscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usersubscription.FieldTeamID)
	}
